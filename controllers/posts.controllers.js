...
const createPost = async (req, res) => {
    try {
        ...
        if (!usuarioEncontrado) {
            return res.status(404).json({ message: `El usuario '${userNickname}' no existe en el sistema.` });
        }
...
const getAllPosts = async (req, res) => {
    try {
        ...
        console.error(error);
        return res.status(500).json({ message: 'Hubo un error al obtener los posts.' });
    }
};
